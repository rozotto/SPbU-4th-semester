...
def task1(df):
    return pd.DataFrame({'Пары': [df.query("HasEvents")['HasEvents'].count()]}).to_excel(writer, sheet_name='Sheet1', index=False)
...
def task2(df):
    ...
    return pd.DataFrame(list(workload.items()), columns=['Name', 'Value']).to_excel(writer,sheet_name='Sheet2', index=False)
...
def task3(df):
    ...
    return pd.DataFrame(list(res.items()), columns=['Name', 'Value']).to_excel(writer,sheet_name='Sheet3', index=False)
...
def task4(df):
    ...
    return pd.DataFrame(list(week.items()), columns=['Day', 'Value']).to_excel(writer,sheet_name='Sheet4', index=False)
        ...
        for day in range(6):
            for lesson in range(len(df['EducatorEventsDays'][lector][day]['DayStudyEvents'])):
                if df['EducatorEventsDays'][lector][day]['DayStudyEvents'][lesson]['IsCanceled']:
                    cnt_IsCanceled += 1
    return pd.DataFrame({'Отменили': [cnt_IsCanceled]}).to_excel(writer,sheet_name='Sheet5', index=False)
...
def task6(df):
    ...
    plt.show()
    return pd.DataFrame(list(ss.items())[-13:], columns=['Time', 'Value']).to_excel(writer,sheet_name='Sheet6', index=False)
...
def task7(df):
    ...
    return pd.DataFrame(list(ss.items())[-3:], columns=['Name', 'Value']).to_excel(writer,sheet_name='Sheet7', index=False)
...
def task8(df):
    ...
    return pd.DataFrame(list(res.items()), columns=['Place', 'Value']).to_excel(writer,sheet_name='Sheet8', index=False)
...
def task9(df):
    ...
    return pd.DataFrame({'Всего групп': [len(groups)]}).to_excel(writer,sheet_name='Sheet9', index=False)
...
def task10(df):
    ...
    plt.pie(ss.values(), labels=ss.keys())
    plt.show()
    return pd.DataFrame(list(ss.items()), columns=['Name', 'Value']).to_excel(writer,sheet_name='Sheet10', index=False)
...
def task11(df):
    ...
    plt.show()
    return pd.DataFrame(list(ss.items()), columns=['Name', 'Value']).to_excel(writer,sheet_name='Sheet11', index=False)
...
def task12(df):
    ...
    return pd.DataFrame(list(courses.items()), columns=['Name', 'Value']).to_excel(writer,sheet_name='Sheet12', index=False)
...
def task13(df):
    ...
    return pd.DataFrame(list(cnt_placesVO.items()), columns=['Name', 'Value']).to_excel(writer,sheet_name='Sheet13', index=False)
